...
import com.aurionpro.payrollsystem.dto.employeeAttendanceLeave.EmployeeLeavesDto;
...
public class LeaveServiceImpl implements LeaveService {
	...
	public LeaveResponseDto applyLeave(Long employeeId, LeaveApplyRequestDto dto) {
	    ...
	    Employee manager = employeeRepository.findById(dto.getManagerId())
	            .orElseThrow(() -> new IllegalArgumentException("Manager not found: " + dto.getManagerId()));
	    ...
	    EmployeeLeaves rem = employeeLeavesRepository.findByEmployeeAndLeaveType(employee, leaveType)
	            .orElse(null);

	    
	    LeaveResponseDto resp = new LeaveResponseDto();
	    resp.setLeaveRequestId(lr.getLeaveRequestId());
	    resp.setEmployeeId(employee.getEmployeeId());
	    resp.setManagerId(manager.getEmployeeId());
	    resp.setLeaveTypeId(leaveType.getLeaveTypeId());
	    resp.setStartDate(lr.getStartDate());
	    resp.setEndDate(lr.getEndDate());
	    resp.setLeaveStatus(LeaveStatus.PENDING);
	    resp.setReason(lr.getReason());
	    resp.setRemainingDays(rem); 
	...
	public List<LeaveResponseDto> getLeavesForEmployee(Long employeeId) {
		...
		for (LeaveRequest lr : list) {
			EmployeeLeaves rem = employeeLeavesRepository
					.findByEmployeeAndLeaveType(lr.getEmployeeId(), lr.getLeaveTypeId()).orElse(null);
			// Long mgrId = lr.getManagerId() != null ? lr.getManagerId().getEmployeeId() :
			// null;
...
//            dto.setLeaveTypeId(lr.getLeaveTypeId().getLeaveTypeId());
	...
	public List<LeaveResponseDto> getLeavesForManager(Long managerId) {
		...
		for (LeaveRequest lr : list) {
			EmployeeLeaves rem = employeeLeavesRepository
					.findByEmployeeAndLeaveType(lr.getEmployeeId(), lr.getLeaveTypeId()).orElse(null);
			result.add(new LeaveResponseDto(lr.getLeaveRequestId(), lr.getEmployeeId().getEmployeeId(),
					lr.getManagerId() != null ? lr.getManagerId().getEmployeeId() : null,
					lr.getLeaveTypeId().getLeaveTypeId(), lr.getStartDate(), lr.getEndDate(), lr.getLeaveStatus(),
					lr.getReason(), rem));
		}
		return result;
	}
	...
	public LeaveResponseDto decideLeave(LeaveDecisionDto dto, Long managerId) {
		...
		if (dto.getLeaveStatus() == LeaveStatus.APPROVED) {
			...
			java.sql.Date sqlStartDate = java.sql.Date.valueOf(lr.getStartDate());
			java.sql.Date sqlEndDate = java.sql.Date.valueOf(lr.getEndDate());

			long diffInMs = sqlEndDate.getTime() - sqlStartDate.getTime();
			long days = (diffInMs / (1000 * 60 * 60 * 24)) + 1;

			String leaveTypeName = lr.getLeaveTypeId().getLeaveTypeName().toLowerCase();

			boolean isUnpaidLeave = leaveTypeName.contains("unpaid") || leaveTypeName.contains("loss of pay");

			if (!isUnpaidLeave) {
				EmployeeLeaves empLeaves = employeeLeavesRepository
						.findByEmployeeAndLeaveType(lr.getEmployeeId(), lr.getLeaveTypeId())
						.orElseThrow(() -> new IllegalStateException(
								"No leave balance configured for this employee and leave type"));

				if (empLeaves.getRemainingCount() < days) {
					throw new IllegalStateException(
							"Employee does not have enough leave balance. Remaining: " + empLeaves.getRemainingCount());
				}
			...
			while (!date.isAfter(lr.getEndDate())) {
				...
				Attendance a = attendanceRepository.findById(aid).orElse(new Attendance(aid, lr.getEmployeeId(),
						isUnpaidLeave ? AttendanceStatus.UNPAID_LEAVE : AttendanceStatus.LEAVE));

				a.setAttendanceStatus(isUnpaidLeave ? AttendanceStatus.UNPAID_LEAVE : AttendanceStatus.LEAVE);
		...
		return new LeaveResponseDto(lr.getLeaveRequestId(), lr.getEmployeeId().getEmployeeId(),
				lr.getManagerId() != null ? lr.getManagerId().getEmployeeId() : null,
				lr.getLeaveTypeId().getLeaveTypeId(), lr.getStartDate(), lr.getEndDate(), lr.getLeaveStatus(),
				lr.getReason(), rem);
	}
